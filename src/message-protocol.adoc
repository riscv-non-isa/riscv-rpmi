...
below summarizes all RPMI message types.
...
an RPMI request message may carry data required to perform the control and
...
microcontroller to the application processors which is used to inform the latter
...
! SERVICE_ID
! Service ID. +
8-bit wide identifier representing an RPMI service. This identifier
is unique within a given RPMI service group.

! [15:0]
! SERVICEGROUP_ID
! Service group ID. +
16-bit wide unique identifier representing an RPMI service group.
!===
...
! TOKEN
! Message token. +
16-bit number for an RPMI message.
...
The message data format for an RPMI request message and an RPMI acknowledgement
message is defined separately for each RPMI service. The message data format
for an RPMI notification message is defined in the <<Notifications>>.
...
the maximum message data size can use multi-part RPMI acknowledgement messages.
...
associated with it based on the type of an RPMI event. Any action required
...
RPMI service group must provide an `ENABLE_NOTIFICATION` service with fixed
`SERVICE_ID=0x01`, which application processors can use to enable or disable
notification messages for a particular RPMI event. By default, notifications
are disabled for all RPMI events of an RPMI service group. The platform
microcontroller sends RPMI notification messages only for RPMI events which are
enabled by the application processors. If multiple RPMI events are supported by
an RPMI service group, then the application processors must enable each RPMI
event individually.
...
included in the RPMI message data.
...
| RPMI_ERR_ALREADY
| -6
| Operation already in progress, or state already changed for the intended
operation.
