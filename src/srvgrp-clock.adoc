...
==== Clock Rate Format
Each clock rate is an array of two 32-bit values `(uint32, uint32)` represented
...
All supported clocks in the system are designated by a 32-bit integer identifier
called `CLOCK_ID`.
...
When the `REMAINING` field is not `0`, the application processor must
...
In Auto mode, the platform can autonomously choose a supported rate closest to the requested rate.
----
!===
...
! RPMI_ERR_INVALID_PARAM
! `CLOCK_ID` or `CLOCK_RATE` is invalid or the flags passed are invalid or reserved.
