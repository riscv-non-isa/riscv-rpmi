...
Clock ID identifiers are sequential and start from `0`.

The device or group of devices sharing the same clock source form a single clock
...
format is of discrete type, the `NUM_RATES` field returns the number of discrete
clock rates supported by the clock. In case of linear range format the `NUM_RATES`
...
| NUM_RATES
| uint32
| Number of discrete clock rates in case of discrete format. Or, the
  number of linear ranges in case of linear format.
...
The total words required for the number of discrete clock rates or linear ranges
according to the format in one message must not exceed the total words available
in a message DATA field. If the format is linear range and a clock supports
multiple linear ranges, then only complete linear ranges must be returned as per
the data format of the linear range described in <<clock-rate-format-linear>>.

If the total words required to store all supported discrete clock rates or the
linear ranges exceed the available words in message DATA field then `REMAINING`
...
| REMAINING
| uint32
| Remaining number of discrete clock rates in case of discrete format. Or, the
  remaining number of linear ranges in case of linear format.

| 3
| RETURNED
| uint32
| Number of discrete clock rates returned in case of discrete format. Or,
 the number of linear ranges returned in case of linear format.
