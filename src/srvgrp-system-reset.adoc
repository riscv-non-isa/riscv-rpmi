...
flags indicate if a `RESET_TYPE` is supported, except for System Shutdown and
System Cold Reset, which are mandatory and supported by default. System Warm
Reset support can be discovered with this service.
...
The application processor must request only supported reset types discovered
