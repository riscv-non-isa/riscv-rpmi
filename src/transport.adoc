...
of the RPMI shared memory transport. The layout and attributes of an RPMI shared
...
in an RPMI shared memory transport.
...
NOTE: Out of the total `M` slots, `(M - 2)` slots are used as message queue
slots to store RPMI messages. The `(slot index - 2)` index value
...
then the message consumer must wait for messages to be available. If all message
slots in the RPMI shared memory queue are occupied, then the message producer must
...
queues must be of the same size, and hence have an equal number of slots (or
queue capacity). The size of each RPMI shared memory queue must be a multiple
of the slot size.
...
a bottleneck for message communication. It is recommended that the number of
...
NOTE: If the platform supports PLIC, it needs to provide an MMIO register to
inject an edge-triggered interrupt.
...
may also contain other set bits that must persist on every write to the doorbell
register.
...
* Must provide physical address and other attributes (such as optional
  fast-channel doorbell) of the fast-channels via a service defined by
  the service group.
