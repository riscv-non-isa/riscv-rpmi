...
The platform may have multiple application processors that share actual
performance controls, such as clocks and voltage regulators, depending on the
platform. In such cases, a performance level change for one application processor
will affect the entire group sharing the controls. It is the responsibility of
...
be assigned both types of fast-channel.

===== Performance Request Fast-channel
In this fast-channel, the application processor will either write the desired
performance level in case of normal mode or the minimum and maximum performance
level in case of autonomous (CPPC2) mode. Otherwise, the application processor
can call the `CPPC_WRITE_REG` service for the `DesiredPerformanceRegister` or
`MinimumPerformanceRegister` and `MaximumPerformanceRegister`.

The supported values in this fast-channel, depending on the CPPC mode (either
normal or autonomous), are discoverable through the `CPPC_GET_FAST_CHANNEL_REGION`
service.
...
In this fast-channel, the application processor will read the supported value
for estimating the delivered performance as performance feedback for an
application processor. The application processor's current frequency (Hz) is
used for the performance feedback. The platform microcontroller must write the
frequency of an application processor in the fast-channel whenever it changes.
...
`CPPC_GET_FAST_CHANNEL_REGION` service. The `base-address` of the shared memory
region must be aligned to `8 bytes`, which is the maximum size of a fast-channel in
both the types.
...
region can be discovered through the `CPPC_GET_FAST_CHANNEL_OFFSET` service. The
...
NOTE: The write value may also contain other set bits, which must persist on every
...
This service is used to probe a CPPC register implementation status for an
...
fast-channel of an application processor.
...
If the fast-channels are supported, the application processor must write only
the desired performance level to the fast-channel, instead of writing to the
`DesiredPerformanceRegister` through this service. Similarly, in autonomous
mode, the application processor must write the minimum and maximum limit
levels to the fast-channel instead of calling this service for the
`MinimumPerformanceRegister` and `MaximumPerformanceRegister`. Otherwise,
...
return only the Hart IDs which can be accommodated. The application processor
