...
Multi-linear range format can be supported by having multiple `linear range` tuples
...
voltage levels in case of discrete format and number of linear range tuples in case
of linear range format. Each domain can support only one voltage level format.
...
! RPMI_ERR_INVALID_PARAM
! Voltage `DOMAIN_ID` or `VOLTAGE_LEVEL_INDEX` is invalid.
...
| REMAINING
| uint32
| The remaining number of discrete voltage levels if the format is discrete type,
or the number of linear range tuples if the format is linear range.
