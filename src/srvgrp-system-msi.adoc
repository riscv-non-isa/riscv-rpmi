...
The number of system MSIs supported by this service group is fixed and is
referred to as `SYS_NUM_MSI`. Each system MSI is associated with a unique index,
referred to as `SYS_MSI_INDEX`, where `0 <&#8203;= SYS_MSI_INDEX < SYS_NUM_MSI`.
...
The system MSI state is 32-bit word, referred to as `SYS_MSI_STATE`, which
includes whether the system MSI is enabled or disabled and whether it is
...
The platform microcontroller can send a pending system MSI only if it is
enabled and configured with a valid MSI target address. The system MSI can be
enabled or disabled using the `SYSMSI_SET_MSI_STATE` service, whereas the
...
MSI target address is `setipnum` register of an APLIC domain, then the
application processors receive it as a wired interrupt.
...
! `SYS_MSI_INDEX` value is greater than or equal to `SYS_NUM_MSI`.
!===
- Other errors <<table_error_codes>>.
...
! RPMI_ERR_INVALID_PARAM
! `SYS_MSI_INDEX` value is greater than or equal to `SYS_NUM_MSI` or
