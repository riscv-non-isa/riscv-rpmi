...
| 1 	| START_ADDR_LOW | uint32	| Lower `32 bit` of start address.
| 2 	| START_ADDR_HIGH | uint32	| Higher `32 bit` of start address.
|===
...
| 2	| RESUME_ADDR_LOW | uint32	| Lower `32 bit` of resume address. + 
_Only used for non-retentive suspend types._
| 3	| RESUME_ADDR_HIGH | uint32	| Higher `32 bit` of resume address. +
_Only used for non-retentive suspend types._
|===
...
| 5	| SUSPEND_TYPE[N - 1]  | uint32	| Suspend Type
|===
