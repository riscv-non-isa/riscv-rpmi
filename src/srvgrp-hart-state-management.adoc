...
From a hart perspective, a start state means that the hart has started
executing instructions, and a stop state means that the hart is not executing
instructions. The platform can implement the stop state either by powering
down the hart or just putting the hart in a platform supported low-power state.
...
This service returns success if the platform microcontroller has successfully
...
into a quiesced state, such as WFI, which can be detected and allows the platform
microcontroller to stop the hart. The mechanism to detect the hart quiesced state
by the platform microcontroller is platform specific.
...
state. The target hart, after successful acknowledgement, must enter into a
quiesced state, such as WFI, which can be detected and allows the platform
microcontroller to complete the suspend state transition. The mechanism to detect
the hart quiesced state by the platform microcontroller is platform specific.

For a non-retentive suspend state, the hart will resume its execution from the
