...
management, since system and CPU power management are handled by already defined
service groups such as SYSTEM_RESET, SYSTEM_SUSPEND and HART_STATE_MANAGEMENT.

A domain can have a single device if its power states can be controlled
independently, or it can have multiple devices if they share the same
power control lines and their power states can only be changed collectively.
...
context. The level of context preserved depends on the level of power state.
...
.Request Data
[cols="1, 3, 2, 6", width=100%, align="center", options="header"]
|===
| Word
| Name
| Type
...
the implementation, but generally, they include values such as ON, OFF and
vendor specific values. Refer <<section-power-state>> for more details.
|===
...
specific values. Refer <<section-power-state>>
for more details.
|===
