...
This service group defines services used to request the platform microcontroller
to transition the system into a suspend state, also called a sleep state.
The suspend state `SUSPEND_TO_RAM` is supported by default by the platform.
If the application processor requests `SUSPEND_TO_RAM`, it is implicit to the
platform microcontroller that all the application processors, except the
requesting one, are in `STOPPED` state and that necessary state saving in RAM has
been completed.
...
If a `SUSPEND_TYPE` supports a custom resume address, the platform must
configure it for the resuming application processor.
...
system to a suspend state. This service returns successfully when the platform
...
The application processor must request only supported suspend types discovered
using the `SYSSUSP_GET_ATTRIBUTES` service.

If a suspend type does not support a custom resume address as discovered by the
application processor through the `SYSSUSP_GET_ATTRIBUTES` service, then the
`RESUME_ADDR_LOW` and `RESUME_ADDR_HIGH` will be ignored, and the application
processor will resume from the `pc` (program counter) after the instruction
that put the application processor in the quiesced state, such as `WFI`.
