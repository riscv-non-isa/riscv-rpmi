...
responsible for directly controlling voltages and clocks, this mechanism
operates on a metric less integer performance scale. Each integer value on this
...
level index does not need to be contiguous or on a linear scale. For
example, the mapping between levels index and frequency can be as
straightforward as using a multiplication factor of `1000` or an ascending index
...
* The shared memory region designated for fast-channels across performance
  ...
  bytes) of this shared memory region can be obtained through the
  `PERF_GET_FAST_CHANNEL_REGION` service. The base address of the shared memory
...
* The `PERF_GET_FAST_CHANNEL_REGION` provides physical memory for Performance
  Service Group. The offset of the physical address retrieved in
  `PERF_GET_FAST_CHANNEL_ATTRIBUTES` of the 'Performance Domain ID and
  Service ID' pair is based on the starting address in
...
* The doorbell register address is the physical address of the register.
  ...
  NOTE: The write value may also contain other set bits which must persist
        on every write to the doorbell register.
...
send a notification to the client whenever there is a change in the performance
...
This service provides a list of available performance levels, also called
...
`performance level index` returned by this service.
...
Refer to <<section-perf-level-attribute>> for the complete structure of
...
.Response Data
[cols="1, 3, 1, 6a", width=100%, align="center", options="header"]
|===
| Word
| Name
| Type
