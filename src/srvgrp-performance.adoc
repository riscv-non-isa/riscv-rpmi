...
operates on an metric-less integer performance scale. Any value on this scale is
called a `Performance Level` which also can also be referred to as the
performance operating point. What this scale represents and the metric is
entirely platform dependent. The values on this scale are discrete, and the
platform has full control over mapping these performance points to hardware
