...
running a trusted OS that provides, directly or via hosted Trusted Applications
...
Throughout this chapter, REEs and TEEs mean the same as described by the Global
...
assumption that a TEE and its hypervisor run only when explicitly invoked by an
REE (and/or its hypervisor) or in order to serve interrupts it has registered.

This yields an execution model in which the REE domain routinely controls all
...
The implementation of the TEE service group in hypervisors and M-Mode firmware
is referred to as the TEE framework, or simply framework.
...
All endpoints, proxied or physical, are given a unique identity by the framework.
The mechanism by which this happens is outside of the scope of this document.
...
they are physical or proxied, TEEs or REEs; in all such cases, the word
endpoint is used throughout the document instead of explicitly calling out
whether the reference is to a TEE or an REE, a physical or proxied entity.

When referring to physical entities rather than generic endpoints, the REE and
TEE terms will be used explicitly.
...
This target type allows an endpoint to acquire information about itself from
the framework. This can be used by a newly created REE or TEE to discover if
the bootloaders which predated it in the current VM shared or lent memory for
...
further message to it and, for TEEs, any further interrupts.
...
the framework does not guarantee either the hart or the time at which the signal
...
If the target endpoint returns any error code other than RPMI_SUCCESS or
...
If the target endpoint returns any error code other than RPMI_SUCCESS, the
...
This service is used by an endpoint to retrieve its pending signals from the
framework.
...
own access rights to it. The memory can be later reclaimed with the original
access rights.
...
represented as a scatter-gather list comprising of 3 fields, with lengths
...
| BLOCK_HIGH[M]
| uint32
| Array of memory blocks. Upper 32-bit, encoded as follows
...
! [31:0]
! ADDRESS_HIGH
! Upper 32-bit of the physical or guest-physical address of the i-th block, in
number of 4kB pages.
!===

| BLOCK_LOW[M]
| uint32
| Array of memory blocks. Lower 32-bit, encoded as follows
[cols="1,2, 5a", options="header"]
!===
! Bits
! Field
...
! [31:12]
! ADDRESS_LOW
! Lower 20-bit of the physical or guest-physical address of the i-th block, in
number of 4kB pages.
...
In this call, the endpoint passes the memory parcel identifier
representing the memory it is accepting, together with associated information
...
| CREATOR_ID
| uint32
| Identifier of the endpoint which invoked the TEE_MEMORY_PARCEL_CREATE, leading
to the creation of the memory parcel being accepted by this service.
...
returned address. The framework will return error if the hint, when requested,
cannot be satisfied.
...
| ADDRESS_HIGH
| uint32
| Upper 32-bit of the address at which the framework must return the memory.
...
| ADDRESS_LOW
| uint32
| Lower 32-bit of the address at which the framework must return the memory.
...
| MAX_PAGES
| uint32
| Maximum size of the memory (in number of pages) that the caller is ready to
accept.
...
flag), this list must contain the identifiers of all endpoints, other than the
acceptor, listed in the call to TEE_MEMORY_PARCEL_CREATE that created this
memory parcel, excluding the identifier of the endpoint that created the
parcel itself, whose identifier and access is passed in CREATOR_ID and
CREATOR_ACCESS and are always validated by the framework.
...
Any error returned other than the above will be interpreted by the framework as
...
This service is used to send data synchronously from an endpoint (in an REE or
TEE) to a TEE.
